 ...
 * @author Ben Gunter
 */
public abstract class VFS { //虚拟文件系统
  private static final Log log = LogFactory.getLog(VFS.class);

  /** The built-in implementations. */
  public static final Class<?>[] IMPLEMENTATIONS = { JBoss6VFS.class, DefaultVFS.class }; //两个虚拟文件系统实现类

  /** The list to which implementations are added by {@link #addImplClass(Class)}. */
  public static final List<Class<? extends VFS>> USER_IMPLEMENTATIONS = new ArrayList<Class<? extends VFS>>(); //记录用户自定义的VFS实现类

  /** Singleton instance. */
  private static VFS instance; //单例模式，记录全局唯一的VFS
  ...
  public static VFS getInstance() {
    if (instance != null) { //检测对象
      return instance;
    }

    // Try the user implementations first, then the built-ins
    List<Class<? extends VFS>> impls = new ArrayList<Class<? extends VFS>>();
    impls.addAll(USER_IMPLEMENTATIONS);//添加用的VFS实现类
    impls.addAll(Arrays.asList((Class<? extends VFS>[]) IMPLEMENTATIONS)); //添加系统的VFS实现

    // Try each implementation class until a valid one is found 遍历impls 集合，依次实例化VFS 对象并检测VFS 对象是否有效，一旦得到有效的VFS 对象，则结束循环
    VFS vfs = null;
    for (int i = 0; vfs == null || !vfs.isValid(); i++) {
      Class<? extends VFS> impl = impls.get(i); //获取VFS实现类
      try {
        vfs = impl.newInstance(); //创建实例
